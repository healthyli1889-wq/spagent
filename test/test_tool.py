...
Usage:
    ...
    # Test Wan video generation (text-to-video)
    python test/test_tool.py --tool wan --image dummy --prompt "A cat playing with yarn" --duration 5 --size 1280*720

    # Test Wan video generation (image-to-video)
    python test/test_tool.py --tool wan --image assets/dog.jpeg --prompt "The dog starts running" --duration 5
    ...
    python test/test_tool.py --tool wan --image dummy --prompt "test video" --use_mock
"""
...
# ============================================================
# Wan Video Generation Tool Test
# ============================================================
...
def test_wan(
    ...
    """
    Directly test Wan video generation tool.
    ...
    logger.info("=" * 60)
    logger.info("Wan Video Generation Tool Test")
    logger.info("=" * 60)
    logger.info(f"  Prompt           : {prompt[:80]}...")
    logger.info(f"  Image path       : {image_path or 'None (text-to-video)'}")
    logger.info(f"  Duration         : {duration}s")
    ...
    tool = WanTool(use_mock=use_mock)
    ...
    if not result.get("success"):
        logger.error(f"Wan tool failed: {result.get('error', 'unknown error')}")
        return None

    logger.info("Wan video generation succeeded!")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["pi3", "pi3x", "depth", "segmentation", "detection", "veo", "sora", "wan"],
    ...
    vid_group = parser.add_argument_group("Video generation options (Veo / Sora / Wan)")
    ...
    vid_group.add_argument(
        ...
        type=str,
        default="1280*720",
        help="Video resolution for Wan in 'width*height' format (default: '1280*720').",
    )
